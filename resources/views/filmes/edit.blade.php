...
<body>
    <form action="{{ route('filmes.update', ['filme' => $filme]) }}" method="post">
        @csrf
        @method('PUT')
        <div>
        ...
        <div>
            ...
            <input type="text" name="ano" id="inputAno" value="{{ old('ano', $filme->ano) }}">
            @error('ano')
            <em>{{ $message }}</em>
            @enderror
        </div>
        <div>
            ...
            <input type="text" name="cartaz_url" id="inputCartaz" value="{{ old('cartaz_url', $filme->cartaz_url) }}">
            @error('cartaz_url')
            <em>{{ $message }}</em>
            @enderror
        </div>
        <div>
            ...
            <textarea name="sumario" id="inputSumario" rows=10>{{ old('sumario', $filme->sumario) }}</textarea>
            @error('sumario')
            <em>{{ $message }}</em>
            @enderror
        </div>
        <div>
            ...
            <input type="text" name="trailer_url" id="inputTrailer" value="{{ old('trailer_url', $filme->trailer_url) }}">
            @error('trailer_url')
            <em>{{ $message }}</em>
            @enderror
        </div>
